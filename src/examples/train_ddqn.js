/**
 * AutonoLearn RL — Double DQN Training Script
 *
 * Trains a Double Deep Q-Network agent on the Flappy Bird environment.
...
const logStream = fs.createWriteStream(path.join(resultsDir, 'ddqn_log.txt'), { flags: 'w' });
const _origWrite = process.stdout.write.bind(process.stdout);
process.stdout.write = (chunk, ...rest) => { logStream.write(chunk); return _origWrite(chunk, ...rest); };

// ─── Parameters ───────────────────────────────────────────────
const TOTAL_EPISODES      = 30000;
...
console.log(`\n${'═'.repeat(60)}`);
console.log(`  AutonoLearn RL — Double DQN Training`);
console.log(`  Episodes: ${TOTAL_EPISODES} | Gap: ${CONFIG.pipe.gap}`);
console.log(`  Timestamp: ${timestamp}`);
console.log(`${'═'.repeat(60)}`);

const agent   = new DoubleDQNAgent();
...
const ddqnDir    = path.join(modelsDir, 'ddqn');
fs.mkdirSync(ddqnDir,    { recursive: true });
...
await agent.save(tf.io.withSaveHandler(async (artifacts) => {
    ...
    fs.writeFileSync(path.join(ddqnDir, 'model.json'), JSON.stringify(modelJSON));
    if (artifacts.weightData) {
        fs.writeFileSync(path.join(ddqnDir, 'weights.bin'), Buffer.from(artifacts.weightData));
    }
    return { modelArtifactsInfo: { dateSaved: new Date(), modelTopologyType: 'JSON' } };
}));
console.log(`  💾 Modelo guardado en models_final/${difficulty}/ddqn/`);
...
fs.writeFileSync(path.join(resultsDir, 'ddqn_results.json'), JSON.stringify(resultData, null, 2));
console.log(`  📊 Resultados guardados en results/ddqn_results.json\n`);
